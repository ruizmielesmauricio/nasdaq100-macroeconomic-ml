# -*- coding: utf-8 -*-
"""
NASDAQ-100 + Macroeconomic Indicators ML Project


Author: Mauricio Ruiz
Original created: Sun Jul 10 17:29:14 2025

What changed vs original:
- Uses pathlib paths so it works locally and on GitHub Actions runners
- Forces matplotlib headless backend (no GUI required)
- Replaces all plt.show() with plt.savefig(...) into results/figures/
- Writes all CSV outputs into data/
...
import matplotlib
matplotlib.use("Agg")  # required for GitHub Actions / headless runs
...
# ------------------------------------------------------------
# Paths (IMPORTANT)
# If this script is located at src/main.py, ROOT should be repo root:
#   ROOT = Path(__file__).resolve().parents[1]
# If this script is located at main.py in repo root, use:
#   ROOT = Path(__file__).resolve().parent
# ------------------------------------------------------------
ROOT = Path(__file__).resolve().parents[1]  # <-- assumes src/main.py
...
print(f"Missing values (Top20): {missing_values}")

# Reset index -> Have Date as a column to merge with NDX
clean_data.reset_index(inplace=True)

# Save to csv
clean_data.to_csv(DATA_DIR / "nasdaq_top20_data.csv", index=False)
...
# getting Nasdaq Closing Price - NDX
ndx = yf.download('^NDX', start=start, end=end)
...
# Save to CSV
ndx.to_csv(DATA_DIR / "ndx_with_returns.csv", index=False)

# Load NDX return target and merge by Date
ndx = pd.read_csv(DATA_DIR / "ndx_with_returns.csv")
...
merged_data.to_csv(DATA_DIR / "nasdaq_top20_with_target.csv", index=False)
...
end_date_dt = datetime(2024, 12, 31)

# Federal Funds Rate
fed_rate = web.DataReader('FEDFUNDS', 'fred', start_date, end_date_dt)
# Consumer Price Index (CPI)
cpi = web.DataReader('CPIAUCSL', 'fred', start_date, end_date_dt)
# Unemployment Rate
unemployment = web.DataReader('UNRATE', 'fred', start_date, end_date_dt)
# Real GDP (Quarterly)
gdp = web.DataReader('GDPC1', 'fred', start_date, end_date_dt)
# 10-Year Treasury Constant Maturity Rate
ten_year = web.DataReader('GS10', 'fred', start_date, end_date_dt)
# M2 Money Supply
m2 = web.DataReader('M2SL', 'fred', start_date, end_date_dt)
...
print("Missing values (raw FRED):")
print(fred_data.isnull().sum())

# Plot missing values (save to file instead of plt.show())
...
plt.tight_layout()
plt.savefig(FIG_DIR / "missing_gdp.png", dpi=300, bbox_inches="tight")
plt.close()

# Forward fill for missing values in GDP (deprecation-safe)
fred_data = fred_data.sort_index().ffill()

# 2 missing values at the beginning so using bfill
fred_data = fred_data.ffill().bfill()

print("Missing values (FRED after ffill+bfill):")
print(fred_data.isnull().sum())

# Save the merged dataset
fred_data.to_csv(DATA_DIR / "fred_combined_data.csv")
...
fred_data = pd.read_csv(DATA_DIR / "fred_combined_data.csv", parse_dates=['DATE'])
merged_data = pd.read_csv(DATA_DIR / "nasdaq_top20_with_target.csv", parse_dates=['Date'])

# Rename date column
fred_data.rename(columns={'DATE': 'Date'}, inplace=True)

# Merge macro with nasdaq
fred_data = fred_data.sort_values('Date').set_index('Date')
...
# Save CSV
full_data.to_csv(DATA_DIR / "final_modeling_dataset.csv", index=False)
...
# Binary flag GDP: Identify when GDP changed (new report)
...
print("Missing values (regression features):")
...
# NOTE: original code used 'Close' (without underscore), which may drop unintended columns.
# Keeping your original behavior would be: ['_Open', '_High', '_Low', '_Volume', 'Close'].
# More precise is to drop '_Close' only. We'll use the precise option here:
columns_to_drop = [
    col for col in regression_df.columns
...
# -----------------------
# Data Analysis Section (SAVE plots)
# -----------------------
macro_vars = [
    'FedFunds_lag1',
    'CPI_lag1',
...
# Plot one chart per macro indicator (save to files)
# Your original used regression_df.index as x; here we use the Date column if present.
# After dropping, Date may still exist; if not, use index.
x_axis_dates = regression_df['Date'] if 'Date' in regression_df.columns else regression_df.index
...
for macro in macro_vars:
    ...
    ax1.plot(x_axis_dates, regression_df[macro], color='blue', label=macro)
    ax1.set_ylabel(macro, color='blue')
    ax1.tick_params(axis='y', labelcolor='blue')

    ax2 = ax1.twinx()
    ax2.plot(x_axis_dates, regression_df['NDX_Return_Tomorrow'], color='orange', alpha=0.6, label='NDX Return (Tomorrow)')
    ax2.set_ylabel('NDX Return (Tomorrow)', color='orange')
    ax2.tick_params(axis='y', labelcolor='orange')

    fig.tight_layout()
    plt.savefig(FIG_DIR / f"ndx_returns_vs_{macro}.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

# Save to csv
regression_df.to_csv(DATA_DIR / "macro_regression_dataset.csv", index=False)
...
print("Missing values (classification features):")
...
# Save to CSV
classification_df.to_csv(DATA_DIR / "market_classification_dataset.csv", index=False)
...
# NOTE: regression_df should contain Date; if Date was dropped earlier, sorting will fail.
# We kept Date by using precise drop rules; if it's missing, comment this line out.
...
print(f"Best iteration: {getattr(xgb_reg, 'best_iteration', 'N/A')}")

# Predictions vs Actual (save)
...
plt.grid(True)
plt.tight_layout()
plt.savefig(FIG_DIR / "reg_pred_vs_actual.png", dpi=300, bbox_inches="tight")
plt.close()

# Training vs validation (early stop) (save)
...
if 'validation_0' in results and 'rmse' in results['validation_0']:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(FIG_DIR / "reg_early_stopping_curve.png", dpi=300, bbox_inches="tight")
    plt.close()

# High importance features (save)
...
plt.tight_layout()
plt.savefig(FIG_DIR / "reg_feature_importance_top10.png", dpi=300, bbox_inches="tight")
plt.close()

# Low importance features (save)
importance_df = pd.DataFrame({
    'Feature': X.columns,
    'Importance': xgb_reg.feature_importances_
...
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig(FIG_DIR / "reg_feature_importance_bottom10.png", dpi=300, bbox_inches="tight")
plt.close()

# Comparison predictions vs actual over time (save)
...
plt.grid(True)
plt.tight_layout()
plt.savefig(FIG_DIR / "reg_actual_vs_pred_over_time.png", dpi=300, bbox_inches="tight")
plt.close()
...
y_cls = classification_df['NDX_Direction_Tomorrow']
...
print(f"Best iteration: {getattr(xgb_clf, 'best_iteration', 'N/A')}")
...
plt.tight_layout()
plt.savefig(FIG_DIR / "clf_confusion_matrix.png", dpi=300, bbox_inches="tight")
plt.close()
...
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig(FIG_DIR / "clf_feature_importance_top10.png", dpi=300, bbox_inches="tight")
plt.close()

# Training vs validation (early stop) (save)
results_cls = xgb_clf.evals_result()
if 'validation_0' in results_cls and 'logloss' in results_cls['validation_0']:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(FIG_DIR / "clf_early_stopping_curve.png", dpi=300, bbox_inches="tight")
    plt.close()
...
plt.barh(importance_df['Feature'], importance_df['Importance'], color='steelblue')
plt.xlabel("Gain Importance")
plt.title("Bottom 10 Feature Importances (by Gain)")
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig(FIG_DIR / "clf_feature_importance_bottom10.png", dpi=300, bbox_inches="tight")
plt.close()

# Comparison predictions vs actual (save)
...
plt.grid(True)
plt.tight_layout()
plt.savefig(FIG_DIR / "clf_actual_vs_pred_directions.png", dpi=300, bbox_inches="tight")
plt.close()

# Prediction confidence histogram (save)
...
plt.grid(True)
plt.tight_layout()
plt.savefig(FIG_DIR / "clf_confidence_histogram.png", dpi=300, bbox_inches="tight")
plt.close()
